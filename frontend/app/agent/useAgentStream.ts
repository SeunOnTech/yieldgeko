...
// ── Inline types (mirrors packages/agent/src/orchestrator/types.ts) ───────────
...
export type EventType    =
  ...
  | 'SAFETY' | 'EXECUTION' | 'POSITION' | 'BREAKERS' | 'LOG' | 'TICK_END';
...
export interface Opportunity {
  ...
  asset: string; tvlUSD: number; grossAPY: number; netAPY: number;
  ...
  minTier: RiskTier; score: number; verifiedOnChain: boolean;
  llamaPoolId: string; address: string; updatedAt: number;
}

export interface Position {
  id: string; venueId: string; venueName: string; protocol: string;
  strategyType: StrategyType; entryAPY: number; entryUSD: number; entryTime: number;
  ...
  effectiveAPY: number; peakUSD: number; drawdownPct: number;
  daysHeld: number; simulated: true;
}

export interface CircuitBreaker {
  id: string; name: string; status: CBStatus;
  value: string; threshold: string; description: string;
}
...
export interface PnLPoint { ts: number; totalUSD: number; navUSD: number; incomeUSD: number }
...
const DEFAULT: StreamState = {
  agent: null, connected: false, lastEvent: null,
  allocation: null, safetyCheck: null,
};
      ...
      try {
        ...
        setStream(prev => {
          ...
          switch (type) {
            ...
            case 'POSITION': {
              if (!next.agent) return next;
              const { userId: uid, position } = payload as any;
              if (!uid || !next.agent.users[uid]) return next;
              return { ...next, agent: { ...next.agent, users: { ...next.agent.users, [uid]: { ...next.agent.users[uid], position } } } };
            }
            ...
            case 'EXECUTION': {
              ...
              return {
                ...
                agent: {
                  ...
                  users: {
                    ...next.agent.users,
                    [uid]: { ...u, executions: [record as ExecutionRecord, ...u.executions].slice(0, 50) },
                  },
                },
              };
            }
            ...
            case 'TICK_END': {
              ...
              return {
                ...
                agent: {
                  ...next.agent,
                  users:     updatedUsers,
                  tickCount: tick ?? next.agent.tickCount,
                  lastTickAt: Date.now(),
