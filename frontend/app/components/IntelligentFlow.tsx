  ...
  return (
    <div style={{ marginTop: 14 }}>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 48 }} preserveAspectRatio="none">
        <defs>
          <linearGradient id="ifChartFill" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#4ADE80" stopOpacity="0.15" />
            <stop offset="100%" stopColor="#4ADE80" stopOpacity="0" />
          </linearGradient>
        </defs>
        <path d={fill} fill="url(#ifChartFill)" />
...
function ParticleCanvas({ width, height }: { width: number; height: number }) {
  ...
  useEffect(() => {
    ...
    const particles = Array.from({ length: 38 }, () => ({
      ...
      r: Math.random() * 1.2 + 0.3, speed: Math.random() * 0.4 + 0.1,
      alpha: Math.random() * 0.35 + 0.05, drift: (Math.random() - 0.5) * 0.2,
    }))
    ...
    const draw = () => {
      ...
      for (let ring = 0; ring < 3; ring++) {
        ...
        const r = age * 220, alpha = (1 - age) * 0.18
        ctx.beginPath(); ctx.arc(cx, cy, r, 0, Math.PI * 2)
        ctx.strokeStyle = `rgba(234,88,12,${alpha})`; ctx.lineWidth = 1; ctx.stroke()
      }

      
      nodes.forEach((n, i) => {
        const pulse = 0.5 + 0.5 * Math.sin(t * 0.006 + n.phase)
        ctx.beginPath(); ctx.arc(n.x, n.y, 1.5 + pulse * 1.5, 0, Math.PI * 2)
        ctx.fillStyle = `rgba(234,88,12,${0.15 + pulse * 0.25})`; ctx.fill()
        const next = nodes[(i + 1) % nodes.length]
        const dist = Math.hypot(next.x - n.x, next.y - n.y)
        if (dist < 250) {
          ctx.beginPath(); ctx.moveTo(n.x, n.y); ctx.lineTo(next.x, next.y)
          ctx.strokeStyle = `rgba(234,88,12,${0.04 + pulse * 0.04})`; ctx.lineWidth = 0.5; ctx.stroke()
        }
      })
      ...
      particles.forEach(p => {
        ...
        ctx.fillStyle = `rgba(234,88,12,${p.alpha})`; ctx.fill()
      })
...
export function IntelligentFlow() {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', maxWidth: 1060, margin: '0 auto', height: 540, overflow: 'hidden' }}>
      <ParticleCanvas width={1060} height={540} />

      
      <div style={{
        position: 'absolute', top: 26, left: '50%', transform: 'translateX(-50%)',
        width: 300, background: 'rgba(18,18,20,0.95)',
        border: `1px solid ${cardActive === 0 ? 'rgba(234,88,12,0.5)' : 'rgba(255,255,255,0.07)'}`,
        borderRadius: 18, padding: '18px 20px', zIndex: 15,
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <img src="https://icons.llamao.fi/icons/protocols/uniswap?w=48&h=48" alt="Uniswap" style={{ width: 22, height: 22, borderRadius: 6 }} />
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'rgba(74,222,128,0.10)', borderRadius: 20, padding: '3px 9px', border: '1px solid rgba(74,222,128,0.2)' }}>
            <div style={{ width: 5, height: 5, borderRadius: '50%', background: '#4ADE80', animation: 'ifPulse 1.5s ease-in-out infinite' }} />
      ...
      <div style={{
        position: 'absolute', top: 26, right: 48, width: 218,
        background: 'rgba(18,18,20,0.95)',
        border: `1px solid ${cardActive === 1 ? `${d.color}55` : 'rgba(255,255,255,0.07)'}`,
        borderRadius: 16, padding: '14px 16px', zIndex: 15,
        boxShadow: cardActive === 1 ? `0 0 14px ${d.color}18` : '0 2px 8px rgba(0,0,0,0.15)',
        transition: 'all 0.5s cubic-bezier(0.16,1,0.3,1)',
      }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.28)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>🧠 Agent Decision</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ marginTop: 12, height: 2, borderRadius: 2, background: 'rgba(255,255,255,0.05)', overflow: 'hidden' }}>
          <div style={{ height: '100%', background: d.color, borderRadius: 2, animation: 'ifScanLine 4s linear infinite' }} />
        </div>
      </div>

      
      <div style={{
        position: 'absolute', top: 26, left: 48, width: 200,
        background: 'rgba(18,18,20,0.95)',
        ...
        <div style={{ fontSize: 22, fontWeight: 800, color: '#4ADE80', fontVariantNumeric: 'tabular-nums' }}>
          {morphoApy.toFixed(1)}%<span style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)', fontWeight: 500, marginLeft: 4 }}>APY</span>
        </div>
      ...
      <div style={{
        position: 'absolute', bottom: 90, left: 48, width: 208,
        ...
        borderRadius: 16, padding: '14px 16px', zIndex: 15,
        boxShadow: harvestFlash ? '0 0 14px rgba(74,222,128,0.08)' : '0 2px 8px rgba(0,0,0,0.15)',
        transition: 'all 0.5s',
      }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.28)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>⚡ Harvest</div>
        ...
        <div style={{ marginTop: 6, fontSize: 18, fontWeight: 800, color: '#4ADE80', fontVariantNumeric: 'tabular-nums' }}>
          {harvestFlash ? '+$4.82' : '+$0.00'}<span style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', fontWeight: 500, marginLeft: 4 }}>net</span>
        </div>
      </div>

      
      <div style={{
        position: 'absolute', bottom: 90, right: 48, width: 218,
        ...
        borderRadius: 16, padding: '14px 16px', zIndex: 15,
        boxShadow: proofFlash ? '0 0 14px rgba(74,222,128,0.07)' : '0 2px 8px rgba(0,0,0,0.15)',
        transition: 'all 0.5s',
      }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.28)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>0G Proof</div>
          {proofFlash && (
            <div style={{ fontSize: 9, fontWeight: 700, color: '#4ADE80', background: 'rgba(74,222,128,0.12)', borderRadius: 4, padding: '2px 6px', border: '1px solid rgba(74,222,128,0.2)' }}>ANCHORED</div>
          )}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <div style={{ width: 6, height: 6, borderRadius: '50%', background: proofFlash ? '#4ADE80' : '#EA580C', animation: 'ifPulse 1.5s ease-in-out infinite', flexShrink: 0 }} />
      ...
      <div style={{ position: 'absolute', bottom: 18, left: 0, right: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, zIndex: 20 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(0,0,0,0.5)', borderRadius: 20, padding: '6px 16px', border: '1px solid rgba(234,88,12,0.2)', backdropFilter: 'blur(8px)' }}>
          <div style={{ width: 5, height: 5, borderRadius: '50%', background: '#EA580C', animation: 'ifPulse 1.2s ease-in-out infinite', flexShrink: 0 }} />
          <span style={{ fontSize: 12, color: '#fff', fontWeight: 600, letterSpacing: '0.01em', minWidth: 200, textAlign: 'left' }}>
            {msg}<span style={{ opacity: 0.5, animation: 'ifBlink 1s step-end infinite' }}>|</span>
          </span>
        </div>
      ...
      <style>{`
        ...
        @keyframes ifBlink    { 0%,100%{opacity:0.5} 50%{opacity:0} }
        @keyframes ifScanLine { 0%{width:0%} 100%{width:100%} }
      `}</style>
    </div>
  )
}
